...
type CatKey = 'H' | 'T' | 'L' | 'P' | 'E' | 'B' | 'G' | 'C' | 'I' | 'X' | 'N'
...
const CATEGORIES: { key: CatKey; icon: string; label: string; color: string }[] = [
  ...
  { key: 'N', icon: '📢',  label: '공지사항',     color: '#9B4D00' },
]
...
export const BLOCK_CATALOG: Record<CatKey, BlockItem[]> = {
  ...
  I: [
    { code: 'I-01', name: '공지 티커 배너',       desc: '1순위: 국보·보물·문화재 자동 흐름 / 2순위: 공지·법회·행사 요약 (자동전환) · 전 사찰 필수 자동포함', tags: ['티커', '국가유산', '공지', '배너', '필수'] },
  ...
  N: [
    { code: 'NS-01', name: '공지스와이프형',        desc: '3카드 스와이프 · 5초 자동전환 · 어드민 연동 · 전 사찰 필수 자동포함', tags: ['공지', '스와이프', '자동전환', '필수'] },
    { code: 'GNB-01', name: 'GNB 네비게이션',       desc: '메뉴 확정: 사찰소개·공지사항·기도법회행사·오시는길 + 🍔 햄버거: 카카오톡·유튜브·네이버블로그', tags: ['GNB', '메뉴', 'SNS', '필수'] },
  ],
}
