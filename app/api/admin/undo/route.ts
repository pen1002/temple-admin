...
import {
  getUndo, clearUndo,
  saveEventList, saveRitualTimes, saveDharma,
  setRaw, delRaw,
} from '@/lib/kv'
  ...
  try {
    ...
    if (!undoData) {
      return NextResponse.json(
        { error: '취소할 수 있는 작업이 없습니다. (30초가 지났습니다)' },
        { status: 400 }
      )
    }
    ...
    if (type === 'notice') {
      ...
      if (notices[0]) await setRaw(`${slug}:notice_1`, notices[0])
      else await delRaw(`${slug}:notice_1`)
      if (notices[1]) await setRaw(`${slug}:notice_2`, notices[1])
      else await delRaw(`${slug}:notice_2`)
      if (notices[2]) await setRaw(`${slug}:notice_3`, notices[2])
      else await delRaw(`${slug}:notice_3`)
    } else if (type === 'event') {
      ...
      await setRaw(`${slug}:gallery_recent`, undoData.gallery)
    }
