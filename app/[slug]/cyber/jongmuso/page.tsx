      ...
      {page === 'front' ? (<>
        ...
        {younggaRows.map((y: any, i: number) => (
          <div key={i} style={{ display: 'grid', gridTemplateColumns: '28px 60px 1fr 130px 36px', gap: 6, alignItems: 'center', padding: '8px 0', borderBottom: `1px solid ${A}11` }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}><input value={y.name} onChange={e => updateYg(i, 'name', e.target.value)} placeholder="성명" style={{ flex: 1, background: 'rgba(255,255,255,0.06)', border: `1px solid ${A}33`, borderRadius: 4, color: '#F5E6C8', padding: '5px 8px', fontSize: 13 }} /><span style={{ color: `${A}88`, fontSize: 12, whiteSpace: 'nowrap' }}>영가</span></div>
