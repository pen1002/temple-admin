...
export default function TemplestayBlock({ temple, config }: Props) {
  ...
  const defaultPrograms: TSProgram[] = [
    { icon: '🌿', name: '비자숲길 걷기 (휴식형)', desc: '일상에 지친 몸과 마음을 달래는 호젓한 자유 포행. 수령 300년 비자나무 숲이 사찰을 병풍처럼 감싸 안고 있습니다.' },
    { icon: '🍵', name: '다담과 사찰 음식', desc: '자연이 주는 가장 순수한 맛의 식사와 스님과의 차담. 야생 차나무에서 얻은 차로 다선일미(茶禪一味)를 체험합니다.' },
    { icon: '🧘', name: '선명상 (체험형)', desc: '타종, 108배, 참선을 통해 번뇌를 비우고 내면을 채우는 전통 수행 체험. 새벽 예불 참여 가능.' },
  ]
