...
// 사찰명 기반 텍스트 마스크 — 컴포넌트 내부 useEffect에서 생성
function buildTextMask(tName: string, cols: number, rows: number): boolean[] {
  ...
  cx.fillStyle = '#fff'
  let fontSize = Math.floor(cv.height * 0.55)
  cx.font = `900 ${fontSize}px 'Apple SD Gothic Neo','Malgun Gothic',sans-serif`
  cx.textAlign = 'center'; cx.textBaseline = 'middle'
  const tw = cx.measureText(tName).width
  if (tw > cv.width * 0.88) {
    fontSize = Math.floor(fontSize * (cv.width * 0.88 / tw))
    cx.font = `900 ${fontSize}px 'Apple SD Gothic Neo','Malgun Gothic',sans-serif`
  }
  cx.fillText(tName, cv.width / 2, cv.height / 2)
  cx.fillText(tName, cv.width / 2 + 0.6, cv.height / 2)
  cx.fillText(tName, cv.width / 2, cv.height / 2 + 0.6)
  const imgData = cx.getImageData(0, 0, cv.width, cv.height).data
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      let sum = 0
      for (let dy = 0; dy < 8; dy++) {
        for (let dx = 0; dx < 8; dx++) { sum += imgData[((row * 8 + dy) * cv.width + col * 8 + dx) * 4] }
      }
      mask[row * cols + col] = (sum / 64) > 55
    }
  }
  return mask
}
...
export default function FiveHundredLanternHeroBlock({ config }: Props) {
  ...
  const tName = (config?.templeName as string) || '사찰명'
  const currentRound = (config?.currentRound as number) || 1
  const MAX = (config?.targetCount as number) || ROUND_TARGETS[currentRound - 1] || 500
  ...
  useEffect(() => {
    ...
    // 사찰명 기반 텍스트 마스크 동적 생성
    const textMask = buildTextMask(tName, COLS, ROWS)
    slotsRef.current.forEach((s, i) => { s.isText = textMask[i] ?? false })
    ...
    const draw = (ts: number) => {
      ...
      if (cur.length >= MAX) {
        ...
        ctx.fillText(`${currentRound}차 ${MAX.toLocaleString()}등 원만성취`, W / 2, H * 0.05)
        ctx.restore()
      }
    ...
    try {
      await fetch('/api/indung', {
        ...
        body: JSON.stringify({
          ...
          lantern_count: 1, contact: contact.trim(),
          phase,
        }),
      })
      await fetchDonors()
  ...
  const shareKakao = () => {
    ...
    const text = `${tName} 인등불사 ${currentRound}차에 동참했습니다.\n\n함께 소원을 빌어요 🕯\n${url}`
        ...
        <div style={{ maxWidth: 420, margin: '0 auto 6px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 4 }}>
            ...
            <div style={{ flex: 1, height: 7, background: 'rgba(255,200,80,0.1)', borderRadius: 4, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${pct}%`, background: 'linear-gradient(90deg,#c9a84c,#f6e05e)', borderRadius: 4, transition: 'width 0.8s' }} />
            </div>
          ...
          {[1,2,3,4,5,6].map(r => {
            ...
            return (
              <div key={r} style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                <span style={{
                  width: active ? 13 : 9, height: active ? 13 : 9, borderRadius: '50%', display: 'inline-block',
                  background: done ? '#c9a84c' : active ? '#fff' : 'rgba(255,255,255,0.12)',
                  border: active ? '2px solid #c9a84c' : done ? 'none' : '1px solid rgba(255,255,255,0.18)',
                ...
                {r < 6 && <span style={{ width: 6, height: 1, background: done ? 'rgba(201,168,76,0.35)' : 'rgba(255,255,255,0.08)' }} />}
              </div>
            )
          })}
        </div>
        <p style={{ color: 'rgba(255,200,80,0.28)', fontSize: 11 }}>
