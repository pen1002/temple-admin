...
## 2026-04-18 재검토: 실제 DB 테이블 대조 결과

대표님이 main DB SQL Editor에서 실행한 `information_schema.tables` 결과와 대조.

### 결과: 14개 모델 = 14개 테이블 **완전 일치**

| 분류 | 모델 수 | 테이블명 확인 | RLS SQL 일치 |
|------|---------|-------------|-------------|
| TENANT_STRICT | 8 | ✅ 전량 확인 | ✅ 전량 정확 |
| TENANT_NORMAL | 2 | ✅ 전량 확인 | ✅ 전량 정확 |
| PUBLIC_READ | 1 | ✅ 확인 | ✅ 정확 |
| GLOBAL | 3 | ✅ 전량 확인 | — (RLS 미적용) |

### 컬럼 수 차이 발견 (참고)

| 모델 | Prisma 필드 | DB 컬럼 | 차이 | 영향 |
|------|-----------|---------|------|------|
| Temple | 32 | 39 | +7 | RLS 무관 (Phase 2 정밀 조사) |
| Believer | 27 | 32 | +5 | RLS 무관 |

Prisma에 미정의된 DB 컬럼이 존재 — 레거시 또는 Supabase 자동 생성 추정.
RLS 정책은 WHERE 조건만 사용하므로 영향 없음.

### 최종 판정

어제 세션 1 인벤토리의 14개 모델 분류·격리 필요도·RLS 정책 타입 **전량 유효**.
교정 불필요.
